...
@router.post("/api/csrftoken", response_model=Csrf)
...
async def login(response: Response, user: UserBody, request: Request, csrf_protect: CsrfProtect = Depends()):
    ...
    response.set_cookie(key="access_token", value=f"Beare {token}", httponly=True, samesite="none", secure=True)
...
def get_user_refresh_jwt(request: Request, response: Response):
    ...
    response.set_cookie(key="access_token", value=f"Beare {new_token}", httponly=True, samesite="none", secure=True)
